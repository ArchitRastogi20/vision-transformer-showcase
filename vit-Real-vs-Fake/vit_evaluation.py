...
def load_model(model, checkpoint_path, device='cuda'):
    ...
    val_prec = checkpoint.get('val_precision', None)

    print(f"Model loaded from: {checkpoint_path}")
    print(f"Epoch from the checkpoint: {epoch}")

    if isinstance(val_acc, (float, int)):
        print(f"Val Accuracy: {val_acc:.4f}")
    else:
        print("Val Accuracy: N/A")

    if isinstance(val_prec, (float, int)):
        print(f"Val Precision: {val_prec:.4f}")
    else:
        print("Val Precision: N/A")
